...
float readLGXFloat(Eip_Session *session, Eip_Connection *connection, char *varName, int numToRead){
		int i;
    printf("Reading %s\n",varName);
    float val_float;
...
// 	BYTE path[]={1,0};
	...
	// 	float val_float;
		...
		if (connection!=NULL)
		{
			...
			char *fluidLevelTag = "val_FluidLevel";
			char *intakeTemperatureTag = "val_IntakeTemperature";
			char *intakePressureTag = "val_IntakePressure";
			float val_FluidLevel = readLGXFloat(session,connection,fluidLevelTag,1);
			float val_IntakeTemperature = readLGXFloat(session,connection,intakeTemperatureTag,1);
			float val_IntakePressure = readLGXFloat(session,connection,intakePressureTag,1);
...
//            }
			...
			if (res!=E_Error)	printf("");//printf("Forward_Close %s\n",cip_err_msg);
